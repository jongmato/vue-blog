          ...
          <v-list-item-content>
            <v-list-item-title :class="$store.state.editable ? 'pl-4':''">
              ...
              <span v-if="$store.state.editable">
                ...
                <v-btn icon @click="moveItem(items.subItems, j, -1)" v-if="j > 0"><v-icon>mdi-chevron-double-up</v-icon></v-btn>
                <v-btn icon @click="moveItem(items.subItems, j, 1)" v-if="j < items.subItems.length -1"><v-icon>mdi-chevron-double-down</v-icon></v-btn>
              </span>
            </v-list-item-title>
          </v-list-item-content>
...
export default {
  ...
  methods: {
    ...
    moveItem (items, i, direction) {
      // const item = items.splice(i, 1)[0]
      // items.splice(i + arrow, 0, item)
      items.splice(i + direction, 0, ...items.splice(i, 1))
      this.save()
    }
  }
}
